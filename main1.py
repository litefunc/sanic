...
app.static('/static', '/mnt/hgfs/共享資料夾/linux/sanic/static')

from urllib import parse
from pandas import *
# from numpy import *  #can not import all in sanic, don't know why
import numpy as np
from sqlite3 import *
...
def template(tpl, **kwargs):
    env = Environment(loader=PackageLoader('main1', 'templates'),autoescape=select_autoescape(['html', 'xml', 'tpl']))
...
@app.route("/testmaco")
async def testmaco(request):
    mytemplate = Template(filename='/home/ak66h1989/Dropbox/linux/sanic/templates/test.html')
    return html(mytemplate.render(name="jack"))
...
def PostParameters(request, data):
    requestUrl = request.raw_args[data]
    print(requestUrl)
...
d = dict()
@app.route('/')   # should contain '/' in tail
async def index(request):
    ...
    return template('testlist.html', d=d, request_url=request.url)  #d is a dictionary, should appear in template
...
async def listfieldajax(request):
    ...
    tab ='#tabs-1'
    d['tab'] = tab
    dbtable = request.raw_args['data'].split('=')[1]
...
async def listfield1ajax(request):
    ...
    tab ='#tabs-5'
    d['tab'] = tab
    dbtable1 = getPostParameter(PostParameters(request, 'data'), 'dbtable1')
...
async def listfield2ajax(request):
    ...
    tab ='#tabs-7'
    d['tab'] = tab
    dbtable2 = getPostParameter(PostParameters(request, 'data'), 'dbtable2')
...
async def listfield3ajax(request):
    ...
    tab ='#tabs-7'
    d['tab'] = tab
    dbtable3 = getPostParameter(PostParameters(request, 'data'), 'dbtable3')
...
#     print("args:", request.args, "url:", request.url, "query_string:", request.query_string, "json:", request.json)
#     print("raw_args:", request.raw_args, "form:", request.form, 'scheme:', request.scheme)
#     args = RequestParameters()
#     print('args:', args)
#     print('args.get("data"):', args.get('data'))
#
#     pars = PostParameters(request, 'data')
...
#     print('pars:', pars, 'width:', width, 'height:', height, 'rangeselector:', rangeselector, 'title:', title, 'cols:', cols)
...
async def scaleajax(request):
    ...
    if request.args.get('value') == 'raw':
        ...
        li = np.array(mll1[id]['df1']).tolist()
...
async def mpajax(request):
    ...
    L = []
    cols1 = getPostParameter(PostParameters(request, 'data'), 'cols1')
...
async def plot1ajax(request):
    global i, L, tab
    print(i)
    # cols = getPostParameter(PostParameters(request, 'data'), 'plot1')
...
async def changeallajax(request):
    global mllys, mllys1, d, k, comp
    mllys, mllys1 =[], []
    compid = getPostParameter(PostParameters(request, 'data'), 'compid')
    print('compid:', compid)
...
async def repajax(request):
    ...
    conn = connect('{}.sqlite3'.format(database))
    c = conn.cursor()
    compid = getPostParameter(PostParameters(request, 'data'), 'compid_report')
    print(compid)
    ...
    for table in ['綜合損益表-一般業', '資產負債表-一般業']:
        ...
        l = np.vstack((np.array([list(df)]), np.array(df))).transpose().tolist()
        m = df.max().max()
        list(df)
        ...
        for i in range(len(df1)):
            for j in range(df1.shape[1]):
                try:
                    ...
                    pass
        lw = np.vstack((np.array([list(df1)]), np.array(df1))).transpose().tolist()
        lc = np.vstack((np.array([list(df2)]), np.array(df2))).transpose().tolist()
        np.shape(lc)
        for i in lw:
            i[0]=0.0
        for i in lc:
...
async def rep1ajax(request):
    ...
    conn = connect('{}.sqlite3'.format(database))
    c = conn.cursor()
    companyId = getPostParameter(PostParameters(request, 'data'), 'companyId')
    ...
    l = np.vstack((np.array([list(df)]), np.array(df), np.array([lspan]))).transpose().tolist()
    lspan = [None for i in list(df)] # None for percent, width, color
    lp = np.vstack((np.array([list(dfPercent)]), np.array(dfPercent), np.array([lspan]))).transpose().tolist()
    lw = np.vstack((np.array([list(dfWidth)]), np.array(dfWidth), np.array([lspan]))).transpose().tolist()
    lc = np.vstack((np.array([list(dfColor)]), np.array(dfColor), np.array([lspan]))).transpose().tolist()
    for i in lw:
        i[0] = 0.0
    for i in lc:
        i[0] = 'white'
...
async def changeForwebCompany(request):
    ...
    index = read_sql_query("SELECT * FROM '%s' WHERE 指數 LIKE %s" % ('大盤統計資訊', "'建材營造類指數'"), conn).rename(
        columns={'收盤指數': '建材營造類指數'}).drop(['指數', '漲跌(+/-)'], axis=1).replace('--', np.nan).replace('---', np.nan)
    index1 = read_sql_query("SELECT * FROM '%s'" % ('index'), conn).replace('--', np.nan).replace('---', np.nan)
    ...
    forweb['lnmo'] = np.log(forweb['調整收盤價'] / forweb['調整收盤價'].shift(120))
    ...
    def lnReturn(*period):
        for days in period:
            n = 240 / days
            forweb['lnr' + str(days)] = np.log(forweb['調整收盤價'].shift(-days) / forweb['調整收盤價']) * n
    ...
    forweb['sign'] = np.sign(forweb['pch'])
    ...
    def f(df, c):
        ...
        df['sign_{}'.format(c)] = np.sign(df['pch_{}'.format(c)])
    ...
    forweb['OSCsign'] = np.sign(forweb.OSC)
    forweb['gr'] = 0
